...
/**
 * Reads the properties with prefix 'kafka.producer.', 'kafka.consumer.' and
 * 'kafka.' from the provided configuration. Initializes a producer and consumer
 * based on the given value. Queue name is driven from the workflow. It is
 * assumed that the queue name provided is already configured in the kafka
 * cluster.
 * 
 * @author preeth
 *
...
public class KafkaObservableQueue implements ObservableQueue {
	...
	/**
	 * Initializes the kafka producer with the defaults.
	 * Fails in case of any mandatory configs are missing.
	 * @param config
	 */
